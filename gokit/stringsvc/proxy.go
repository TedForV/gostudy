...
// proxy implements StringService, forwarding Uppercase requests to the
...
func (mw proxymw) Uppercase(ctx context.Context, s string) (string, error) {
	response, err := mw.uppercase(ctx, uppercaseRequest{s})
	if err != nil {
		return "", err
...
func proxyingMiddleware(ctx context.Context, instances string, logger log.Logger) ServiceMiddleware {
	// If instance is empty, don't proxy
	if instances == "" {
	...
	// Now, build a single, retrying, load-balance endpoint out of all of
